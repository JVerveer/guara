...
function parseArgs(argv) {
  ...
  npm run load:cbs:gold:inkomen-en-bestedingen -- --dataset 85039NED
  npm run load:cbs:gold:inkomen-en-bestedingen -- --refresh

Options:
  --ensure-schema       Execute generic Gold and Inkomen en bestedingen schemas before loading.
  --dataset 85039NED    Load one Inkomen en bestedingen dataset from Silver.
  --limit 100           Maximum Inkomen en bestedingen Silver datasets to load.
...
async function ensureSchema(client) {
  ...
  await client.query(readFileSync(resolve(process.cwd(), "supabase/gold_inkomen_bestedingen_schema.sql"), "utf8"));
}

async function ensureRuntimeTables(client) {
  await client.query(`
    create table if not exists gold_inkomen_bestedingen.dataset_load_progress (
...
async function getDatasets(client, options) {
  ...
  const result = await client.query(
    `
      ...
      where dd.domain_id = 'inkomen-en-bestedingen'
...
async function upsertDataset(client, dataset) {
  ...
  await client.query(
    `
      insert into gold_inkomen_bestedingen.dim_income_dataset (
        income_dataset_key, dataset_key, dataset_code, dataset_title, dataset_version,
  ...
  const result = await client.query(
    `
      select
        ...
        (select count(*)::bigint from gold_inkomen_bestedingen.fact_income_observation where income_dataset_key = $2) as loaded_fact_rows
    `,
  ...
  const result = await client.query(
    `
      select
        ...
        (select count(*)::bigint
         from gold_inkomen_bestedingen.fact_income_observation
         where income_dataset_key = $3 and source_row_id = $2) as loaded_last_row_facts
    `,
...
async function getDatasetProgress(client, datasetKey) {
  const result = await client.query(
    `
      select *
      from gold_inkomen_bestedingen.dataset_load_progress
      where dataset_key = $1
    `,
    [datasetKey]
  );
...
async function upsertDatasetProgress(client, {
  ...
  await client.query(
    `
      insert into gold_inkomen_bestedingen.dataset_load_progress (
      ...
      on conflict (dataset_key) do update set
        ...
        source_rows_loaded = gold_inkomen_bestedingen.dataset_load_progress.source_rows_loaded + excluded.source_rows_loaded,
        fact_rows_loaded = gold_inkomen_bestedingen.dataset_load_progress.fact_rows_loaded + excluded.fact_rows_loaded,
        bridge_rows_loaded = gold_inkomen_bestedingen.dataset_load_progress.bridge_rows_loaded + excluded.bridge_rows_loaded,
        completed_at = coalesce(excluded.completed_at, gold_inkomen_bestedingen.dataset_load_progress.completed_at),
...
async function upsertMeasures(client, dataset) {
  ...
  for (const row of result.rows) {
    ...
    await client.query(
      `
        insert into gold_inkomen_bestedingen.dim_income_indicator (
          income_indicator_key, measure_key, dataset_key, measure_code, indicator_name,
    ...
    const result = await client.query(
      `
        insert into gold_inkomen_bestedingen.fact_income_observation (
          income_dataset_key, income_indicator_key, dataset_key, measure_key,
        ...
        on conflict (
          ...
          income_dataset_key = excluded.income_dataset_key,
          income_indicator_key = excluded.income_indicator_key,
          unit_key = excluded.unit_key,
          calendar_year = excluded.calendar_year,
          ...
          updated_at = now()
        ${returnFactKeys ? "returning income_observation_key, record_hash" : ""}
      `,
      [
        chunk.map((row) => row.incomeDatasetKey),
        chunk.map((row) => row.incomeIndicatorKey),
    ...
    const result = await client.query(
      `
        insert into gold_inkomen_bestedingen.bridge_income_observation_category (
          income_observation_key, category_key, dimension_code, category_code, category_name
        )
        select *
        from unnest($1::bigint[], $2::bigint[], $3::text[], $4::text[], $5::text[])
          as rows(income_observation_key, category_key, dimension_code, category_code, category_name)
        on conflict (income_observation_key, category_key) do update set
          dimension_code = excluded.dimension_code,
          category_code = excluded.category_code,
          category_name = excluded.category_name
      `,
      [
        chunk.map((row) => row.incomeObservationKey),
...
async function loadDataset(client, dataset, options) {
  console.log(`Loading Inkomen en bestedingen mart from Silver: ${dataset.dataset_id}`);

  try {
    const datasetKey = await upsertDataset(client, dataset);
    if (options.refresh) {
      await client.query("delete from gold_inkomen_bestedingen.fact_income_observation where dataset_key = $1", [datasetKey]);
      await client.query("delete from gold_inkomen_bestedingen.dataset_load_progress where dataset_key = $1", [datasetKey]);
    }
    ...
    while (true) {
      ...
      for (const observationChunk of chunkRows(observations, Math.max(1, options.lookupChunkSize))) {
        ...
        for (const observation of observationChunk) {
          ...
          for (const measure of measuresByRow.get(observation.row_id) ?? []) {
            ...
            factPayloads.push({
              incomeDatasetKey: datasetKey,
              incomeIndicatorKey: measureMeta.goldMeasureKey,
              datasetKey,
        ...
        try {
          ...
          if (hasCategoryBridgeRows) {
            const factKeyByHash = new Map(insertedFacts.rows.map((row) => [row.record_hash, row.income_observation_key]));
            const bridgePayloads = [];
            for (const fact of factPayloads) {
              const incomeObservationKey = factKeyByHash.get(fact.recordHash);
              if (!incomeObservationKey) continue;
      ...
      console.log(`  ${dataset.dataset_id}: read batch complete: +${readBatchFacts} income facts from ${readBatchRows} Silver rows`);
      batchesProcessed += 1;
...
async function loadInkomenBestedingenMart(client, options) {
  await ensureRuntimeTables(client);
  const runId = randomUUID();
  await client.query(
    "insert into gold_inkomen_bestedingen.load_runs (run_id, status, message) values ($1, 'pending', $2)",
    [runId, "Loading Inkomen en bestedingen facts directly from Silver into conformed Gold dimensions."]
  );

  try {
    const datasets = await getDatasets(client, options);
    console.log(`Found ${datasets.length} Inkomen en bestedingen Silver dataset(s) for Gold mart.`);
    let sourceRows = 0;
    let factRows = 0;
    let bridgeRows = 0;
    ...
    await client.query(
      `
        update gold_inkomen_bestedingen.load_runs
        set status = 'complete',
          source_fact_count = $2,
          mart_fact_count = $3,
          bridge_row_count = $4,
          finished_at = now(),
          message = 'Inkomen en bestedingen mart loaded directly from Silver.'
        where run_id = $1
      `,
    ...
    await client.query(
      "update gold_inkomen_bestedingen.load_runs set status = 'failed', finished_at = now(), message = $2 where run_id = $1",
      [runId, error.message]
    ).catch(() => {});
    throw error;
  }
...
async function main() {
  ...
  const client = createPostgresClient({
    applicationName: "guara-cbs-gold-inkomen-bestedingen-loader",
  ...
  try {
    ...
    const result = await loadInkomenBestedingenMart(client, options);
    console.log(`Inkomen en bestedingen mart complete: ${result.factRows} facts from ${result.sourceRows} Silver rows, ${result.bridgeRows} bridge rows, ${result.skippedDatasets ?? 0} dataset(s) skipped`);
  } finally {
    await client.end();
  }
}
