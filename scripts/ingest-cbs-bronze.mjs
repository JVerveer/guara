...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (arg === "--dry-run") options.dryRun = true;
      ...
      console.log(`Usage:
  npm run ingest:cbs:bronze -- --limit 25
  npm run ingest:cbs:bronze -- --query "2007" --limit 50
...
async function qualifyDataset(table, properties, dimensionValuesByKey, recordCount) {
...
async function ingestTable(supabase, table, options) {
  ...
  const qualification = await qualifyDataset(table, properties, dimensionValuesByKey, recordCount);
  ...
  console.log(`Ingested ${datasetId}: ${qualification.years.length} years, ${qualification.geographicLevels.join(", ") || "no levels"}`);
}
