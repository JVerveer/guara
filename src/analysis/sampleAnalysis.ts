...
export const SAMPLE_GAPS: Finding[] = [
  ...
  { title: 'Incomplete Sub-contractor Disclosure', severity: 'Medium', vendor: 'Azure', rec: 'Obtain full sub-processor list and assess fourth-party risk.', article: 'Art. 28(3)(j)', category: 'DORA' },
  ...
  { title: 'No Audit Rights Clause', severity: 'Medium', vendor: 'Datadog', rec: 'Include right-to-audit provision in next contract renewal.', article: 'Art. 28(3)(g)', category: 'DORA' },
  { title: 'Missing Business Continuity Plan', severity: 'Low', vendor: 'Salesforce', rec: 'Request vendor BCP and test recovery objectives alignment.', article: 'Art. 28(3)(c)', category: 'DORA' },
  { title: 'Unsigned DPA', severity: 'High', vendor: 'Twilio', rec: 'Execute Data Processing Agreement with updated SCCs.', article: 'Art. 28(2)', category: 'DORA' },
...
const OUTAGE_SIMULATIONS: Record<string, AnalysisResult['outageSimulation']> = {
  'fintech-payments': {
    provider: 'AWS',
...
export function getSampleAnalysisResult(id?: string): AnalysisResult {
  ...
  return {
    ...
    scenario,
    documents: SAMPLE_DOCS.slice(0, Math.min(SAMPLE_DOCS.length, scenario.documents)),
    ...
    outageSimulation: OUTAGE_SIMULATIONS[scenario.id] ?? OUTAGE_SIMULATIONS.default,
